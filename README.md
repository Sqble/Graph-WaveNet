...
We fetched historical weather data (temperature, precipitation, humidity) for all 207 METR-LA sensors via the [Open-Meteo API](https://open-meteo.com/) and interpolated it to 5-minute intervals. The plots below are generated by `visualize_weather.py`.
...
> **Key insight:** Precipitation shows almost no linear correlation with average traffic speed (+0.01), confirming that weather's value is in flagging *anomalous* conditions rather than explaining everyday congestion patterns.
